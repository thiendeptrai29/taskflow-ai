...
export default function AdminPage() {
  ...
  const handleToggle = async (id: string, name: string) => {
    ...
    try {
      ...
      setUsers(prev =>
        prev.map(user =>
          user.id === id ? { ...user, isActive } as User : user
        )
      );

      toast.success(
        `${t('admin.account')} ${name} ${isActive ? t('admin.activated') : t('admin.disabled')}`
      );
    } catch (err: any) {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        ...
                        >
                          {isActive ? `● ${t('admin.active')}` : `● ${t('admin.locked')}`}
                        </span>
                      </td>
